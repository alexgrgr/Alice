...
def search_employee(smartsheet, query, parameter=0):
    ...
    # implementation is going to take into account only the first matched
    # search. The parameters needed to get the values of the cell on the
    # right are sheetId and rowId. columnId is also needed, but only known
    # in JSON received with get_row
    sheetId = search_res.results[0].parent_object_id
    rowId   = search_res.results[0].object_id

    # [Debug] Data for get_row (parameters to be able to ask for the correct
    # cell)
    ...
    if client is None:
        ...
        if is_cisco(user):
            ...
            string_res = "Pero " + spark.mention(user['displayName'],
                                                 user['personEmail'])
                            + ", usted es empleado de Cisco. No tiene AM."
    else:
        ...
        if is_cisco(user):
            ...
            string_res = "Disculpe, no tenemos información indexada de "
            + client + ". Por favor, contacte con _José Méndez_ o busque \
            manualmente en **Smartsheet** [[Comercial Virtual Sales]\
            (https://app.smartsheet.com/b/publish?EQBCT=9d609817f70b4b199a849da34f5ca213)|\
            [Comercial Non Virtual Sales](https://app.smartsheet.com/b/publish?EQBCT=4403c331493c4e62941333f7069b7d7a)] \
            para más información"
            while sheet <= (total_sheets - 1):
                search_res = smartsheet.Search.search_sheet(sheetId[sheet],
                                                            client)
                # As in search_employee, Result is a
                # smartsheet.models.SearchResult object.
                try:
                    rowId = search_res.results[0].object_id
                    # With the parameters needed to get the entire row,
                    # we request it
                    row = smartsheet.Sheets.get_row(sheetId[sheet], rowId,
                        include='discussions,attachments,columns,columnType')
                    # --The following is a botched job--
                    # JSON is formatted in such a way that the cell where I
                    # know where the data I want is in here:
                    ...
                    + "** es: _" + str(am) + "_ . ¿Es correcta la respuesta?"
                    break
        else:
            # In this case, user is not a partner, nor a Cisco Employee. Cannot
            # see internals
            string_res = "Disculpe " + user['displayName']
            + ", usted no tiene permisos para visualizar datos internos."
    print(string_res)
    return string_res
...
def search_pam(smartsheet, user, partner=None):
    ...
    # user's one. This info can only be disclosed internally, so Alice should
    # check first if this user is a Cisco employee. If asking for own PAM,
    # Alice will check to what partner user belongs to.
    if partner is None:
        # In this case, check if user is partner. If true, retreive its pam
        if is_cisco(user):
            # Maybe is a Cisco employee and asks for its pam,
        ...
        # ----------------------------Problem here----------------------------#
            ...
            search_res = smartsheet.Search.search_sheet(sheetId,
                                                        user['personEmail'])
            rowId = search_res.results[0].object_id
            # With the parameters needed to get the entire row, we request it
            row = smartsheet.Sheets.get_row(sheetId, rowId,
        ...
        if is_cisco(user):
            ...
            try:
                ...
                string_res = "Disculpe, no tenemos información indexada de \
                dicho partner. Por favor, contacte con _José Méndez_ para \
                más información"
            else:
            ...
            # In this case, user is not a partner, nor a Cisco Employee. cannot
            # see internals
            string_res = user['displayName'] + ", usted no tiene permisos \
            para visualizar datos internos."
    print(string_res)
    return string_res
...
def buffer_it(JSON, sbuffer):
    ...
    if JSON['data']['personEmail'] != os.environ.get('BOT_EMAIL',
        ...
        # [WARNING] UUIDV1 specifies string + time ID. Maybe there is need to
        # use roomId as identification, but not very well specified in Docs
